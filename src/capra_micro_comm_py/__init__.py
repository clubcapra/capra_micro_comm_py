...
COMMON_FILE = BASE_PATH / 'common.h'
...
class BinaryData:
    ...
    def parseCls(self):
        ...
        for f in cls._ofmt:
            l = '    '
            if f == '_':
                l += cls._values[i].__class__.__name__ + ' '
            else:
                l += TYPE_MAP[f] + ' '
            if f == 'x':
                n = f'pad{pad}'
...
class CommandHook(Generic[P, R]):
    ...
    def definitionParse(self):
        r = self.returnType.__name__
        p = self.paramType.__name__
        f = self._call.__name__
        
        return f'{r} {f}({p} p)' + '{ ' + f'return {r}' + '{}; }'
    
    def callParse(self, varName:str):
        r = self.returnType.__name__
        p = self.paramType.__name__
        f = self._call.__name__
        
        return f'{r} {varName} = {f}({p}' + '{});'
...
class CommandManager:
    ...
    _pingStart:Union[float,None]=None
    _pingVal:Union[Int,None]=None
    _ping:Union[float,None]=None
    def __init__(self):
        self._id = 0
        self._stream = None
        ...
        @ping.preCall
        def prePing(i:Int):
            CommandManager._pingStart = time.time()
            CommandManager._pingVal = i
        
        @ping.postCall
        def postPing(i:Int):
            now = time.time()
            if CommandManager._pingVal is None or CommandManager._pingStart is None:
                CommandManager._ping = inf
            elif CommandManager._pingVal.i != i.i:
                CommandManager._ping = nan
            else:
                CommandManager._ping = now - CommandManager._pingStart
            CommandManager._pingStart = None
            CommandManager._pingVal = None
    ...
    def generateAPI(self, test:bool=False) -> Tuple[str, int]:
        res = dedent("""\
            #pragma once
    ...
    def generateTestAPI(self):
        ...
        for c in self._commands:
            res += c.definitionParse() + '\n\n'
            calls += '    ' + c.callParse(f'test{c._call.__name__}') + '\n'
    ...
    def testAPI(self, outputDir:Path):
        ...
        commonFile.symlink_to(COMMON_FILE)
        
        apiFile = include / 'api.h'
        apiFile.touch()
        with apiFile.open('w') as wr:
            wr.write(api)
            
        testFile = src / 'test.cpp'
        testFile.touch()
        with testFile.open('w') as wr:
            wr.write(test)
        
        output = build / 'test'
        opt = 'g++ ' + str(testFile) + ' -o ' + str(output) + ' -I' + str(include)
        res = os.system(opt)
        assert res == 0
        
        # output.chmod(555)
        # res = os.system(str(output))
        # assert res == 0
